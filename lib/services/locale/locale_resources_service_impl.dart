...
final class LocaleResourcesServiceImpl implements LocaleResourcesService {
  ...
  @override
  Future<Option<String>> getLastVpnCacheTime() async {
    final time = await secureStorage.read(key: _Keys.lastVpnCacheTime);

    return optionOf(time);
  }

  @override
  Future<void> setLastVpnCacheTime(String time) async =>
      await secureStorage.write(key: _Keys.lastVpnCacheTime, value: time);

  @override
  Future<void> deleteLastVpnCacheTime() async {
    await secureStorage.delete(key: _Keys.lastVpnCacheTime);
  }
}
...
abstract final class _Keys {
  ...
  static const String lastVpnCacheTime = "lastVpnCacheTime";
}
